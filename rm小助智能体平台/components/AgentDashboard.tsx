...
import { PlusIcon, UserGroupIcon, ClockIcon, TrashIcon } from './Icons';
...
const AgentDashboard: React.FC<AgentDashboardProps> = ({ 
  ...
  const filteredAgents = agents.filter(agent => {
    ...
    // 2. Status Filtering
    if (statusFilter === '已发布' && agent.status !== 'published') return false;
    if (statusFilter === '草稿' && agent.status !== 'draft') return false;
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="h-16 bg-white border-b border-gray-200 px-8 flex items-center justify-between shadow-sm sticky top-0 z-10">
        ...
        <div className="flex items-center gap-4">
            <div className="w-8 h-8 rounded-full bg-blue-100 border border-white shadow-sm overflow-hidden flex items-center justify-center text-blue-600 font-bold text-xs">
                U
            </div>
        </div>
      </header>
      ...
      <main className="flex-1 p-8 max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-end justify-between mb-8">
            <div>
                <h2 className="text-2xl font-bold text-slate-800 mb-2">智能体列表</h2>
                <p className="text-sm text-slate-500">
                    当前可见 {filteredAgents.length} 个智能体，{filteredAgents.filter(a => a.status === 'published').length} 个已发布
                </p>
            </div>
            <div className="flex items-center gap-3">
                ...
                <select 
                  ...
                  className="pl-3 pr-8 py-2 rounded-lg border border-gray-200 text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none shadow-sm cursor-pointer"
                >
                    <option>所有状态</option>
                    <option>已发布</option>
                    <option>草稿</option>
                ...
                >
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                        ...
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="font-bold text-slate-800 text-lg line-clamp-1" title={agent.name}>
                              {agent.name}
                          </h3>
                        </div>
